...
const urlImage = {image: urlBase + "images/B", thumbnail: urlBase + "thumbnail/T", link: "https://party.0chem.com/?id="};
...
const formData = {
  ...
  imageClick: (groupId: number, id: number) => {
    ...
    if (item) {
      ...
      if (imageView.value) {
        console.log("image");
        imageView.value.open(url, item.degree + formData.imageData.value.degree, item.url + "更新于 " + item.date, urlImage.link + item.url);
      }
    }
  },
  imageClose: () => {
